    ...
    public:
        ...
        void update(SDL_Renderer* renderer, std::unique_ptr<AssetManager>& assetManager) {
            for (auto entity : getSystemEntities()) {
                ...
                const SDL_Rect tmp = SDL_Rect(static_cast<int>(transform.position.x), static_cast<int>(transform.position.y), static_cast<int>(sprite.w), static_cast<int>(sprite.h));
